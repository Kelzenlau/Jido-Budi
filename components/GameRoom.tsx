...
import { ArrowLeft, Clock, Zap, Play, Loader, RefreshCw, Mail, CheckCircle, Trophy, Gamepad2 } from 'lucide-react';
...
import { AppUser, GameConfig } from '../types';
...
const GameOverScreen: React.FC<{
  ...
  onOpenAuth: () => void;
  onPlayAgain: () => void;
}> = ({ score, voucherCode, user, onPlayAgain }) => {
  ...
  const sendVoucher = async () => {
    ...
    try {
        ...
        // Add to User's private wallet
        if (user.uid) {
            await addDoc(collection(db, 'artifacts', appId, 'users', user.uid, 'vouchers'), voucherData);
            // Add to Admin view
            await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'admin_vouchers'), voucherData);
            setEmailStatus('sent');
        }
    } catch (e) {
        console.error("Error sending voucher:", e);
        setEmailStatus('error');
    }
  };

  useEffect(() => {
    if (voucherCode && user && emailStatus === 'idle') {
      sendVoucher();
    }
  ...
  return (
    <div className="absolute inset-0 bg-slate-900/95 backdrop-blur-md z-30 flex flex-col items-center justify-center rounded-2xl text-center p-6 animate-in fade-in zoom-in duration-300">
      {voucherCode ? (
        <>
          <div className="relative">
            ...
            <Trophy size={56} className="text-yellow-400 mb-4 animate-bounce relative z-10" />
          </div>
          ...
          <div className="bg-gradient-to-r from-yellow-100 to-yellow-50 p-4 rounded-xl w-full border-2 border-yellow-400 border-dashed mb-4 relative overflow-hidden transform hover:scale-105 transition-transform">
            <p className="text-xs font-bold text-yellow-800 uppercase tracking-widest mb-1">{t('voucher_code')}</p>
            <p className="text-2xl font-mono font-black text-slate-900 tracking-wider">{voucherCode}</p>
          </div>
          {user && (
            <div className="flex flex-col items-center w-full mb-6">
              ...
              {emailStatus === 'sent' && <div className="flex flex-col items-center"><div className="flex items-center gap-2 text-green-400 bg-green-400/10 px-4 py-2 rounded-full border border-green-400/20 mb-2"><CheckCircle size={16} /><span className="text-xs font-bold">{t('sent_to')} {user.email}</span></div></div>}
              {emailStatus === 'error' && <button onClick={sendVoucher} className="flex items-center gap-2 text-red-300 bg-red-500/10 px-4 py-2 rounded-full border border-red-400/20 hover:bg-red-500/20"><RefreshCw size={16} /><span className="text-xs font-bold">{t('retry')}</span></button>}
            </div>
          )}
        </>
      ) : (
      ...
      <button onClick={onPlayAgain} className="px-8 py-3 bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-bold rounded-full shadow-lg shadow-cyan-500/30 hover:scale-105 transition-transform">
        {t('play_again')}
      </button>
    </div>
  );
...
const Match3Game: React.FC<GameRoomProps> = ({ user, onOpenAuth, onBack }) => {
  const { t } = useTranslation();
  const WIDTH = 8;
  const [gameConfig, setGameConfig] = useState<GameConfig>(DEFAULT_GAME_CONFIG as GameConfig);
  ...
  const createBoard = useCallback((theme: string) => {
    const randomBoard = [];
  ...
  useEffect(() => {
    getDoc(doc(db, 'artifacts', appId, 'public', 'data', 'config', 'game')).then(s => {
      const config = s.exists() ? s.data() as GameConfig : DEFAULT_GAME_CONFIG as GameConfig;
      setGameConfig(config);
      setTimeLeft(config.timeLimit || 60);
      setBoard(createBoard(config.theme));
    });
  }, [createBoard]);
  ...
  useEffect(() => {
    if (!isPlaying) return;
    const timer = setInterval(() => { if(!checkForMatch()) { moveBelow(); } }, 100);
  ...
  useEffect(() => { 
    if (isPlaying && score >= gameConfig.winScore && !voucherCode) { 
      setIsPlaying(false); 
      setIsGameOver(true); 
      setVoucherCode('MATCH-' + Math.random().toString(36).substr(2, 6).toUpperCase()); 
    } 
  }, [score, isPlaying, voucherCode, gameConfig]);
  ...
  const startGame = () => { 
    ...
    setIsGameOver(false); 
    setScore(0); 
    setTimeLeft(gameConfig.timeLimit || 60); 
    setBoard(createBoard(gameConfig.theme)); 
    setVoucherCode(null); 
  };
    ...
    <div className="flex flex-col items-center justify-center p-4 bg-indigo-50 rounded-3xl shadow-2xl border-4 border-slate-700/50 relative overflow-hidden max-w-lg mx-auto w-full">
      ...
      <div className="w-full flex justify-between items-center mb-4 bg-indigo-900 p-3 rounded-xl border border-white/10 mt-8">
        <div className="flex items-center space-x-2">
            <div className="bg-yellow-400 p-1.5 rounded-lg shadow-lg shadow-yellow-400/20"><Zap size={20} className="text-yellow-900" fill="currentColor" /></div>
            <div><div className="text-xs text-indigo-200 font-bold uppercase tracking-wider">{t('score')}</div><div className="text-xl font-black text-white leading-none">{score}</div></div>
        </div>
        <div className="flex items-center space-x-2">
            <div className="text-right"><div className="text-xs text-indigo-200 font-bold uppercase tracking-wider">{t('timer')}</div><div className={`text-xl font-black leading-none ${timeLeft < 10 ? 'text-red-400 animate-pulse' : 'text-white'}`}>00:{timeLeft < 10 ? `0${timeLeft}` : timeLeft}</div></div>
            <div className="bg-blue-500 p-1.5 rounded-lg shadow-lg shadow-blue-500/20"><Clock size={20} className="text-white" /></div>
        </div>
      </div>
      <div className="relative bg-slate-900/50 p-3 rounded-2xl border border-white/5">
        <div className="grid grid-cols-8 gap-1 sm:gap-2">
          {board.map((candy, index) => (
            <div 
                ...
                onDrop={dragDrop} 
                onDragEnd={dragEnd} 
                className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 flex items-center justify-center text-2xl sm:text-3xl bg-gradient-to-br from-slate-700 to-slate-800 rounded-lg cursor-pointer transition-all duration-200 shadow-sm border border-white/5 hover:scale-105 hover:brightness-110 active:scale-95"
            >
                <span className="drop-shadow-md select-none pointer-events-none">{candy}</span>
            </div>
          ))}
        </div>
        {(!isPlaying && !isGameOver) && (
            <div className="absolute inset-0 bg-slate-900/80 backdrop-blur-sm z-20 flex flex-col items-center justify-center rounded-2xl">
                <h3 className="text-2xl font-bold text-white mb-4">Snack Match</h3>
                <button onClick={startGame} className="px-8 py-3 bg-gradient-to-r from-pink-500 to-purple-600 text-white font-bold rounded-full shadow-lg shadow-pink-500/30 hover:scale-105 transition-transform">{t('start_game')}</button>
            </div>
        )}
        {isGameOver && <GameOverScreen score={score} voucherCode={voucherCode} user={user} onOpenAuth={onOpenAuth} onPlayAgain={startGame} />}
      </div>
    </div>
  );
};

const SnackSwipeGame: React.FC<GameRoomProps> = ({ user, onOpenAuth, onBack }) => {
    const { t } = useTranslation();
    const [score, setScore] = useState(0);
    const [timeLeft, setTimeLeft] = useState(60);
    const [isPlaying, setIsPlaying] = useState(false);
    ...
    const startGame = () => { setIsPlaying(true); setIsGameOver(false); setScore(0); setTimeLeft(gameConfig.timeLimit || 60); setSnacks([]); setVoucherCode(null); lastSpawnTime.current = performance.now(); };
    ...
    return (
        <div className="flex flex-col items-center justify-center p-4 bg-slate-800 rounded-3xl shadow-2xl border-4 border-slate-700/50 relative overflow-hidden max-w-lg mx-auto w-full h-[600px]">
            ...
            <div ref={containerRef} className="relative w-full h-full bg-gradient-to-b from-indigo-900 to-slate-900 rounded-2xl overflow-hidden cursor-crosshair touch-none border border-white/5" onTouchMove={e => handleMove(e.touches[0].clientX)} onMouseMove={e => handleMove(e.clientX)}>
                ...
                <div className="absolute bottom-4 w-16 h-16 transition-transform duration-75 ease-out flex items-center justify-center text-4xl" style={{ left: `${cartX}%`, transform: 'translateX(-50%)' }}>
                   🛒
                </div>
...
export const GameSelection: React.FC<{ onSelect: (game: 'match3' | 'swipe') => void }> = ({ onSelect }) => {
    const { t } = useTranslation();
    return (
